...
class ContactRequestTestCase(TestCase):
    ...
    def test_contact_request_name_max_length(self) -> None:
        contact_request: ContactRequest = ContactRequest(name="My Name" * 100, email="my@test.com",
                                                         content="test content")
    ...
    def test_contact_request_email_required(self) -> None:
        contact_request: ContactRequest = ContactRequest(name="My Name", content="test content")
        with self.assertRaises(ValidationError) as error:
            contact_request.full_clean()
    ...
    def test_contact_request_email_validation(self) -> None:
        contact_request: ContactRequest = ContactRequest(name="My Name", email="mytest.com", content="test content")
        with self.assertRaises(ValidationError) as error:
            contact_request.full_clean()
        msg: str = error.exception.message_dict['email'][0]
    ...
    def test_contact_request_pub_dttm_order(self) -> None:
        ...
        contact_request: ContactRequest = ContactRequest.objects.create(name="My Name", email="my@test.com",
                                                                        content="test content")
